  ...
  const focusedDrillPrompt = (idx: number): { prompt: string; answer: number; tags: SkillTag[] } => {
    ...
    if (d === 2) {
      ...
      const dayTwoDrillPrompts = [
        ...
        `סִפְרוּ מִ-${start} לְ-${end} וְכִּתְבוּ אֶת מִסְפַּר הַסּוֹף.`,
    ...
    if (d === 5) {
      ...
      const dayFiveDrillPrompts = [
        ...
        `בַּחֲרוּ אֶת הַמִּסְפָּר הַגָּדוֹל יוֹתֵר: ${small} אוֹ ${big}.`,
      ];
      return {
    ...
    if (d === 6) {
      if (idx % 2 === 0) {
        ...
        const evenDrillPrompts = [
          ...
          `הוֹסִיפוּ ${b} לְ-${a} וְכִּתְבוּ אֶת הַסְּכוּם.`,
      ...
      const oddDrillPrompts = [
        ...
        `הַחְסִירוּ ${b} מִ-${a} וְכִּתְבוּ תְּשׁוּבָה.`,
      ];
      return {
    ...
    },
    {
      ...
      exercises: [
        ...
        numberInput(
          ...
          8,
          d === 7
            ? "לְגִיל הָיוּ 11 קְלָפִים. הוּא קִבֵּל עוֹד 5. כַּמָּה קְלָפִים יֵשׁ לוֹ?"
