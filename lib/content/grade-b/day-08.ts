import type { DayConcept } from "../engine/day-builder";

export const concept: DayConcept = {
  dayNumber: 8,
  title: "חִלּוּק לִשְׁתֵּי קְבוּצוֹת שָׁווֹת",
  objective: "בְּסוֹף הַיּוֹם הַתַּלְמִיד/ה יְבִין חִלּוּק כְּחִלּוּק שָׁוֶה לִשְׁתַּיִם.",
  mainTags: ["division-equal-groups", "multiplication-intro"],
  spiralReviewTags: ["multiplication-tables", "word-problems"],
  arithmeticPrompt: "חִלְּקוּ 16 סְפָרִים לִשְׁנֵי מַדָּפִים שָׁווִים. כַּמָּה בְּכָל מַדָּף?",
  arithmeticAnswer: 8,
  arithmeticMcOptions: ["6", "8", "10"],
  arithmeticMcAnswer: "8",
  languagePrompt: "בַּחֲרוּ: מְחַלְּקִים 12 לְ-3 קְבוּצוֹת שָׁווֹת — כַּמָּה בְּכָל קְבוּצָה?",
  languageOptions: ["אַרְבַּע", "שָׁלוֹשׁ", "שֵׁשׁ"],
  languageAnswer: "אַרְבַּע",
  reviewPrompt: "אֱמֶת אוֹ שֶׁקֶר: אִם חִילְּקוּ 18 לִשְׁתַּיִם שָׁווֹת — מְקַבְּלִים 9",
  reviewAnswer: true,
  challengePrompt: "20 בָּלוֹנִים בִּשְׁנֵי צְבָעִים, בְּכַמּוּת שָׁוָה. כַּמָּה מִכָּל צֶבַע?",
  challengeAnswer: 10,
};
