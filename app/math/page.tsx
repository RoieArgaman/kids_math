...
export default function MathGradePickerPage() {
  ...
  // כיתה ב׳ נפתחת רק אחרי שעוברים את המבחן המסכם של כיתה א׳.
  // (previewAll = מצב QA שמדלג על השער.)
  const gradeBLocked = !gradeAFinalPassed && !previewAll;
  ...
  return (
    <main data-testid={testIds.screen.gradePicker.root()} className="pb-10">
      <div data-testid={childTid(testIds.screen.gradePicker.root(), "topNav")} className="mb-4 flex items-center justify-between gap-3">
        <AppNavLink data-testid={testIds.screen.gradePicker.navBack()} href={routes.subjectPicker({ previewAll })}>
          חזרה לבחירת נושא
        </AppNavLink>
        <Link
        ...
        {gradeBLocked ? (
          ...
          >
            ...
            <div data-testid={childTid(testIds.screen.gradePicker.gradeCard("b"), "ctaRow")} className="mt-4">
              <p data-testid={childTid(testIds.screen.gradePicker.gradeCard("b"), "lockedHint")} className="muted text-center text-sm">
                סַיְּימוּ אֶת הַמִּבְחָן הַמְּסַכֵּם שֶׁל כִּיתָּה א׳ כְּדֵי לִפְתּוֹחַ 🔒
              </p>
            </div>
          </div>
        ) : (
