...
### 2026-07-26 (Subject final-exam celebration dialog was stuck open) — ULTRA
- **Trigger:** "when finishing final exam it's like stuck in the dialog instead of making any action when dismissing the dialog."
- **What changed / where:** `components/screens/subject/SubjectFinalExamScreen.tsx` (English & Science share it) + regression assertions in `tests/e2e/english-exam-smoke.spec.ts` and `tests/e2e/science-exam-smoke.spec.ts`.
- **What we learned:**
  1. **A modal driven directly off persisted state + a no-op `onConfirm` is un-closable.** The screen rendered `<StarReward visible={submitted && Boolean(state?.passed)} onConfirm={() => {}} />` — the confirm button had nothing to toggle and `visible` was recomputed from `submittedAt` every render, so it also re-trapped the user on reload. `StarReward`'s only exit is its confirm button (no backdrop/Esc dismiss), so a no-op `onConfirm` = permanent trap.
  2. **The three sibling consumers already had the right pattern** (`SectionScreen`, `SubjectSectionScreen`, `SubjectDayScreen`): a local `showReward` boolean set `true` on the completing action and `false` in `onConfirm`. Fix was to adopt it — reward visibility is ephemeral UI state, not something to derive from persisted progress.
  3. **`toBeVisible()` does not detect overlay occlusion.** The existing pass test asserted the finish panel was visible and passed even though the full-screen `aria-modal` `star-reward-overlay` sat on top — that's why the bug shipped. The new guards click the confirm button and assert `starReward.overlay()` `toHaveCount(0)`, plus a reload-after-pass check.
  4. **Only the audio-first subjects were affected.** The math `FinalExamScreen` renders no `StarReward` (inline results), so the bug was scoped to the shared `SubjectFinalExamScreen`.
- **How to reuse:** never bind a dismissible overlay's `visible` to persisted state with an empty `onConfirm`; give it a local `show*` boolean toggled by the same handler. When a component has a modal overlay, a passing `toBeVisible()` on content behind it is not proof the flow is unblocked — assert the overlay is gone after the dismiss action.
