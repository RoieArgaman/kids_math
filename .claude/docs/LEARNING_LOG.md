...
### 2026-07-26 (Merge 8 E2E shard reports → publish to GitHub Pages) — MAX
- **Trigger:** "add playwright report that takes the results from all the e2e shards together and pushes it to GitHub Pages" — on commit **and** on merge.
- **What changed / where:** `playwright.config.ts` (CI-gated `blob` reporter), `.github/workflows/ci.yml` (per-shard `blob-report-<shard>` upload in the `e2e` matrix + new non-blocking `e2e-report` job that merges and publishes), `package.json` (`test:e2e:merge` helper), docs.
- **What we learned:**
  1. **The publish job MUST be `continue-on-error: true`.** `deploy.yml` gates Firebase on CI's `workflow_run.conclusion == 'success'`, so any new failing job in `ci.yml` blocks the app deploy. Same pattern already used by `content-accuracy` and `npm audit`. Single most important constraint of the change.
  2. **Gate `blob` to CI only** — `reporter: process.env.CI ? [["list"],["blob"]] : [["list"]]` — so local `npm run test:e2e` is unchanged (verified: no `blob-report/` locally; blob active under `CI=1`).
  3. **Sharded blob output path:** `npx playwright test --shard=i/n` writes `blob-report/report-<i>.zip` relative to the config **rootDir** (repo root in CI) — where `upload-artifact path: blob-report/` points. Filenames are already per-shard-unique, so `download-artifact merge-multiple: true` into one dir then `merge-reports --reporter html` is collision-free. Proved the full shard→merge→HTML pipeline locally with a synthetic 2-shard config before pushing.
  4. **Upload blobs with `if: !cancelled()`** so a **failed** run still publishes a report — a red run is exactly when the report matters.
  5. **Single Pages site, "latest wins":** publishing on every commit/PR and on merge means the one live site reflects the most recent run per ref (no per-PR isolation). Fork PRs can't get `pages:write`/`id-token`, so the merged HTML is always also uploaded as a downloadable artifact fallback. Per-ref `concurrency` (`cancel-in-progress`) stops same-branch deploy backlog.
  6. **Two one-time repo prereqs (not code):** Settings→Pages Source = "GitHub Actions", and the `github-pages` environment must allow non-`main` branches or on-commit deploys are rejected. Documented in `DEPLOYMENT.md`.
- **How to reuse:** for any "merge sharded Playwright + publish" task in a repo whose deploy gates on CI success — add the reporter as a CI-only ternary, upload one artifact per shard with a unique name and `!cancelled()`, and put merge+publish in a **separate, `continue-on-error`** job so it can never wedge the deploy gate. Prove blob→merge locally before touching CI.
