...
- **What changed / where:** `.github/workflows/ci.yml` gained a `changes` job (`dorny/paths-filter@v3`) with a POSITIVE `docs` filter + `predicate-quantifier: every`; its `docs` output gates `lint-and-unit` and `e2e` via `if: needs.changes.outputs.docs_only == 'false'`. Non-code globs = `**/*.md`, `docs/**`, `roadmap/**`, `LICENSE`, and the whole `.claude/**` tree. `security-scan` deliberately left ungated. Documented in `AGENTS.md` Quality Gates. PR #85 (skip-path proven on demo PR #86).
- **What we learned:**
  1. **dorny matches with OR-over-patterns, NOT gitignore last-match-wins — verify semantics in CI, not with local picomatch.** A file matches a filter if it matches ANY single pattern. So a `code` filter of `['**', '!**/*.md', …]` is **always true** (bare `**` matches every file; the `!` lines subtract nothing) — it shipped and CI logged `Filter code = true` for a docs-only diff, silently disabling the skip (fail-SAFE: it over-ran, never wrongly skipped). My local picomatch test passed a whole array to one matcher, which DOES honor negation — a wrong model of dorny. Fix: a **positive** `docs` filter (`**/*.md`, `docs/**`, `roadmap/**`, `LICENSE`, `.claude/**`) with **`predicate-quantifier: every`** = true only when EVERY changed file is a doc (docs-only); gate jobs on `docs_only == 'false'`. Here `every` is CORRECT (opposite of what a `code`/`some` framing would want).
  2. **The secret scan must stay unconditional AND inside the CI workflow.** No `needs`/`if`, so it runs even on docs-only diffs (a credential can't slip in through Markdown). Keeping it in `ci.yml` (not a separate workflow) is also what lets it keep gating `deploy.yml` via `workflow_run` — splitting it out would silently drop the deploy-time secret gate.
  3. **On a `pull_request` event, dorny diffs the WHOLE PR against its base, not the latest commit.** So a docs-only follow-up commit can't demonstrate the skip on a PR that already contains a code change — the cumulative PR diff still has code. To prove the skip you need a PR whose *entire* diff is non-code (demo PR #86 based off the #85 branch). The `.claude/` exclusion covers all of rules/docs/commands/plans/settings.json/launch.json — confirmed nothing CI runs reads `.claude/` (not in tsconfig/next.config, not imported; only a console.log string + a Markdown link reference it).
  4. **Trade-off accepted:** docs/`.claude`-only pushes to `main` still trigger `deploy.yml` (CI succeeds → `workflow_run`), redeploying byte-identical output. Cheap and safe; not worth a deploy-side code guard. `main` is unprotected, so skipped jobs don't wedge a required check.
- **How to reuse:** to skip CI on a path class with dorny, list that class as POSITIVE globs in a `docs`-style filter, add `predicate-quantifier: every`, and gate expensive jobs on `<filter>_only == 'false'`. Do NOT try `['**', '!…']` — dorny OR's patterns so `**` wins. **Verify on a real CI run** (a clean non-code PR), not local picomatch, since dorny's per-pattern OR differs from picomatch's array handling. Keep any always-must-run job (secret scan) ungated, and confirm no CI-executed script reads the excluded paths.
