...
- **What changed / where:** `.github/workflows/ci.yml` gained a `changes` job (`dorny/paths-filter@v3`) whose `code` output gates `lint-and-unit` and `e2e` (`if: needs.changes.outputs.code == 'true'`). Non-code globs = `**/*.md`, `docs/**`, `roadmap/**`, `LICENSE`, and the whole `.claude/**` tree. `security-scan` deliberately left ungated. Documented the exemption in `AGENTS.md` Quality Gates. PR #85.
- **What we learned:**
  1. **Use the DEFAULT `some` quantifier, not `predicate-quantifier: every`.** `code` must be true when *at least one* changed file is non-doc, so a **mixed** diff (code + docs) still runs the full suite. `every` inverts this — it requires *all* files to be non-doc and would wrongly skip tests on any code change that also edits a `.md`. (Caught in self-review after first writing `every`.)
  ...
  3. **dorny is last-match-wins (gitignore-style), verified empirically with picomatch.** `.mdc` is NOT matched by `**/*.md`, so the 12 `.claude/rules/*.mdc` files were already treated as code; a stray `.claude/rules/*.md` would have leaked to "docs". Resolved by excluding the entire `.claude/**` tree instead of special-casing — confirmed nothing CI runs reads `.claude/` (not in tsconfig/next.config, not imported, only a console.log string + a Markdown link reference it).
...
- **How to reuse:** to skip CI on a path class, add it as a `!`-negation to the `changes` job's `code` filter (default quantifier), gate expensive jobs on `needs.changes.outputs.code == 'true'`, and leave any always-must-run job (secret scan) ungated. Verify globs empirically with `picomatch` replicating dorny's last-match-wins before merging, and confirm no CI-executed script reads the excluded paths.
