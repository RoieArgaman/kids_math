...
### 2026-07-24 (Hebrew niqqud/grammar sweep across math + science content) — MAX
- **Trigger:** "review all the questions, math and science… make sure the Hebrew is correct and understandable by kids and adults; find patterns that aren't written well."
- **What changed / where:** 63 files under `lib/content/` (grade-a/day-01..29, grade-b/day-01..29, science/day-06 & day-13, engine/expanded-early, expanded-days, progressive-focus). Text-only — no `dayId`/section/exercise IDs touched, so zero localStorage/progress-migration impact.
- **What we learned:**
  1. **The hand-authored `DayConcept` files drifted from the engine's own correct Hebrew.** Every day's `languagePrompt` opened with `בִּחְרוּ` (not a valid conjugation) while the engine-generated prompts already used the correct imperative `בַּחֲרוּ` — 58 vs 4. When the same word exists in both hand-authored content and generated code, grep the generated side for the *correct* form before assuming a spelling is intentional.
  2. **`grep` byte-exactness beats eyeballing niqqud.** Manual reading under-counted (`לְמְרֻבָּע` looked like 1 occurrence, was 2) and can't see combining-mark order. Build fixes by *capturing* the source token from a real file and the correct target from a file that already has it (skeleton match on consonants only), then surgical single-vowel swaps for prefix errors — never retype vocalized strings.
  3. **Two identical-looking byte orderings of the same word coexisted** (`בַּחֲרוּ` as bet+dagesh+patach vs the Unicode-canonical bet+patach+dagesh). Normalize touched tokens with `unicodedata.normalize("NFC", tok)` — but scope NFC to the *tokens you edited* (skeleton-guarded), not whole files, or you inflate the diff with invisible reorderings (caught this on grade-b/day-15 and reverted to a 1-line change).
  4. **House style is full-spelling-with-niqqud** (`חִיבּוּר`, `רִיבּוּעַ`); reverting those to defective spelling would be a huge wrong diff. Only fix genuine errors (prefix two-sheva like `לְתְגוּבוֹת`→`לִתְגוּבוֹת`, `בִּמִילִים`→`בְּמִילִים`) and internal inconsistencies (`שַׁיָּיךְ`→`שַׁיָּךְ`, `רִבּוּעַ`→`רִיבּוּעַ`), plus one broken phrase (`עִגּוּל פּוֹתַחַת` → `פֶּתַח עָגֹל`).
  5. **Sandbox Playwright mismatch:** `npm ci` pulled @playwright/test 1.58 (browser build 1208) but only chromium-1194 is pre-provisioned. Run E2E via a *non-committed* override config setting `use.launchOptions.executablePath: "/opt/pw-browsers/chromium"` against the production server (`CI=1 … npm run start`); dev-mode compile-on-demand exceeds 10m for the full suite.
- **How to reuse:** for Hebrew content sweeps, quantify each pattern with grep first, capture exact bytes from the repo instead of typing niqqud, keep NFC normalization token-scoped, and only "correct" spellings after confirming the repo's own house style. Verify residual-count == 0 for every bad form and that the correct form's count rose by the same amount.

### 2026-07-24 (Sentence-level Hebrew syntax rewrite + TTS check, math concepts) — MAX
- **Trigger:** follow-up — "find syntactically-incorrect sentences and rephrase them; check the read-aloud works for those changes; be a Hebrew teacher checking sentence by sentence in simple language for kids."
- **What changed / where:** 29 files (grade-a: 05,07,13,17,20,21,23,27; grade-b: 03,04,06,08,09,11,12,13,14,15,16,17,19,21,22,23,24,25,26,27,28). 53 span-precise rewrites of `title/objective/arithmeticPrompt/languagePrompt/reviewPrompt/challengePrompt`, plus one `languageOptions`+`languageAnswer` kept in sync (day-23 שִׁשָּׁה→שֵׁשׁ).
- **What we learned:**
  1. **Hand-authored grade-b prose had real nonsense-word typos the niqqud pass didn't target:** `מְצוּלָבִים` (crossed) for `מְצוּלָעִים` (polygons), `טַבִּית` for `לוּחַ`, `יַזְחִיל`/`תּוֹקְפִים`/`דָּגִימוֹת` (non-words), `מַחִיר מֹחַ` (price of a brain) for `מְחִיר צַעֲצוּעַ`, `דִּגּוּטָלִי` for `דִּיגִיטָלִי`. Deterministic + arithmetic checkers can't see these — only reading each sentence aloud as a teacher does.
  2. **Recurring grammar classes:** gender disagreement (`לִשְׁתֵּי מִדָּפִים`→`לִשְׁנֵי מַדָּפִים`, `סְכוּמִים קְטַנּוֹת`→`קְטַנִּים`), missing relative pronoun (`אֵיזוֹ צוּרָה כָּל הַצְּלָעוֹת שָׁווֹת`→`שֶׁכָּל צְלָעוֹתֶיהָ`), question-word-at-end word order (`…יֵשׁ בְּיַחַד כַּמָּה צְלָעוֹת?`→`כַּמָּה צְלָעוֹת יֵשׁ בְּיַחַד…`), and `בְּעִיָּה`→`בְּעָיָה`.
  3. **When rewriting a T/F or MC prompt, pin the answer first.** Verified every `reviewAnswer`/`languageAnswer`/`challengeAnswer` before editing so wording changes never flip truth value or orphan an option (day-11 `1 מֶטֶר = 10 ס״מ` is intentionally `false` → left as-is; day-23 option and answer changed together).
  4. **Span-precise editing beats whole-file NFC.** First attempt typed NFC `old` strings that didn't byte-match non-canonical source, so a fallback normalized whole files (10+ invisible line changes each). Redo: build a consonant-skeleton regex (`[֑-ׇ]*` between letters, literals for digits/punct, `\s+` for spaces) — same idea as `teachingPrimerHebrewLint.skeleton` — and replace only the matched raw span. Result: 53 edits, 53 insertions/53 deletions, nothing collateral.
  5. **Read-aloud check without a device:** manifest is `{}` so all TTS is runtime `normalizeTextForHebrewTts` → browser Hebrew voice. Ported the normalizer in a node harness and ran all 55 changed strings: assert none of `+ = × ÷ < >` survive and no hyphen sits directly between a Hebrew letter and a digit. All clean (`47 + 6`→`47 פְּלוּס 6`, `בְּ-52`→`בְּ 52`). Removing guillemets `«»` also removes a symbol the voice would mishandle.
- **How to reuse:** dump every string field per file and proofread sentence-by-sentence for nonsense words, gender, relative pronouns, and word order — the automated gates can't. Confirm answers before rephrasing T/F & MC. Edit by skeleton-regex span replacement. Verify TTS by porting `normalizeTextForHebrewTts` over the changed strings and asserting no operator/comparison symbol remains.

### 2026-07-25 (Android-tablet "logs out on Back" — spurious boot-401 teardown) — MAX
- **Trigger:** "on some devices, especially Android tablets, it logs out unexpectedly when clicking Back on anything. Can't reproduce."
- **What changed / where:** `lib/auth/context.tsx` (confirm an ambiguous boot 401 with one ~500ms retry before teardown), `lib/auth/api.ts` (`/me` fetch `cache: "no-store"` + explicit `credentials: "same-origin"`), `app/api/auth/me/route.ts` (`Cache-Control: no-store` on 200/401/500), tests (`contextOrdering.test.tsx` +2 cases, new `back-nav-no-logout.spec.ts`). **No** change to `verifyToken`/`verifySession`/cookie attributes/login/logout — the server revocation boundary is untouched.
- **What we learned:**
  1. **The only path that turns a navigation into a logout is the mount effect trusting a single `/api/auth/me` 401** (`context.tsx`): on a device with a local owner marker it immediately `teardownLocalSession()` (wipes progress + owner + analytics, `setUser(null)`). Android tablets evict bfcache under memory pressure, so hardware-Back does a **full document reload** (not a `pageshow persisted` restore) → AuthProvider remounts → `/me` is re-fetched. A transient cookie omission or a **heuristically-cached** 401 (the route set no `Cache-Control`) then reads as a revocation. Fix is defense-in-depth: make `/me` uncacheable **and** confirm a lone 401 with one retry before destroying anything.
  2. **Softening the client teardown does NOT weaken revocation.** The real boundary is server-side `verifySession(requireVersionCheck:true)` on every data route — a revoked user still can't read/write server data during the ~1s confirm window. Keep the retry **exactly once** so a genuine revocation still logs out promptly; `error`/500 stays inconclusive (no teardown), same as before.
  3. **StrictMode makes a fixed request-counter e2e vacuous.** Next 14 dev defaults `reactStrictMode: true`, so the boot effect double-invokes: the first effect is cancelled (`cancelled` guard fires after its `await`), only the **second** (live) effect's result drives teardown. A "return 401 on call #2" mock is nondeterministic against that. **Anchor the transient window to the first post-arm request** (`windowEnd = Date.now()+300` on first call), sized shorter than the ~500ms confirm delay: it deterministically covers StrictMode's back-to-back boot calls yet ends before the retry. Proved non-vacuous by toggling `if (false && …)` on the retry → spec fails; restore → passes (×2 stable).
  4. **`setIsLoading(false)` lives in the effect's `finally`, so awaiting the retry *before* it keeps `isLoading` true through the confirm window** — the app shows its loading state, not a flash of the login screen. The retry fixes the logout and removes the flicker at once; the only added guard is a `cancelled` re-check after the delay (unmount race).
  5. **Sandbox Playwright browser mismatch recurs:** `@playwright/test` wants build 1208 but only `chromium-1194` is provisioned. Run E2E with `PLAYWRIGHT_CHROMIUM_EXECUTABLE=/opt/pw-browsers/chromium-1194/chrome-linux/chrome` (the config already threads this into `launchOptions.executablePath`); inert in CI, which installs its own matching browser.
- **How to reuse:** never perform an irreversible, data-destroying action on a single ambiguous auth signal — confirm it (one bounded retry) and keep the server-side check as the true boundary. Make identity endpoints `no-store` on **every** branch incl. 401. When e2e-testing a boot effect, remember StrictMode double-fires it in dev and anchor time-boxed mocks to the first request, then prove the test fails without the fix before trusting it.
