...
| **D5** | **Duplicated components.** ✅ **Fixed in 3.5.4b:** the two "centered panel" shells (`LockedGradeScreen` → `CenteredPanel`), the four loading treatments (→ `LoadingPanel`), and the `!important` hero/chip resizes (→ `size` props). 🚦 **The day-card merge is NOT a refactor and is deliberately not done.** Investigated in 3.5.4c: the two cards differ in **data, not styling**. Math `<DayCard>` renders a day-number circle, emoji medallion, state chip and a **score-driven progress bar**; the English/Science card renders title + objective + CTA, because **no per-day score exists in the subject model**. Unifying therefore means either (a) one component with two disjoint modes behind optional props — which hides the divergence rather than resolving it, and is worse than two honest components — or (b) giving English/Science per-day scoring, a **content/product change**. Needs a product decision, not an engineering one. | MEDIUM | 🟡 **Partly fixed (3.5.4b); day-card needs a product call** |
