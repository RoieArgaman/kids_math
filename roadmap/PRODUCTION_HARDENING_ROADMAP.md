...
| ID | Finding | Severity | Phase |
|----|---------|----------|-------|
| **D1** | ✅ **Fixed in 3.5.5** via an opt-in `main.screen-wide` (1120px at `lg`), applied to the five density screens only: Subject Picker (`lg:grid-cols-3` — kills the 2+1 orphan), Math + Subject Home (`lg:grid-cols-2`), Badge Gallery (`lg:grid-cols-3 xl:grid-cols-4`), Parent Dashboard and Admin Users. **Practice screens verified still 720px** (Day Overview measured at 1440px viewport: `mainWidth 720`, no opt-in, no horizontal scroll) — a wide input line helps nobody. TopBar's inner row constrained to 1120px so chrome lines up with content instead of floating to the viewport edge. **Note:** the roadmap's own 3.5.5 task text had over-scoped this to also widen Day Overview; the recorded product decision named five screens, and the decision won. **Deferred:** Admin Users gets the width but not the full `lg` table restructure. | ~~HIGH (UX)~~ | ✅ **Fixed (3.5.5)** |
...
| **D2** | **Token drift from the repo's own documented standard.** `.claude/docs/UI_COMPONENTS.md` fixes card radius at `20px`, yet **10 distinct radii** ship (`2xl`×50, `xl`×28, `3xl`×24, `[22px]`, `[18px]`, `[24px]`, `[20px]`, `[26px]`, `[14px]`, `[13px]`); "interactive purple" has 5+ values (`#6d28d9`×32, `#a78bfa`×16, `#8b75cc`×15, `violet-700`, `violet-900`); card titles use both `--title` and `text-violet-900`; the accent rail is 4px/5px/6px. **Note: the source report said "at least four radii" — it is ten.** ✅ **Fixed across 3.5.1 → 3.5.2c:** one accent ramp, one `--title`, a two-step radius scale, one rail width — plus 78 hardcoded hex copies of `--muted`/`--title` routed through the tokens. | ~~HIGH (UX)~~ | ✅ **Fixed (3.5.1 → 3.5.2c)** |
| **D3** | **Touch targets fall under the project's own 44px rule in clusters.** TopBar login is `px-3 py-1.5` inside an `h-10` bar (physically capped at 40px); Plan day-chips are `min-h-10 min-w-10` (40px) and a child taps them to open a day; Admin Users rows carry up to 7 actions at `px-3 py-1.5 text-xs` (~30px); login + admin inputs ~36–40px. **Fixed in 3.5.3 — and the report undercounted.** A systemic sweep (`tests/e2e/touch-targets.spec.ts`, which walks *every* visible interactive element rather than a remembered list) found a whole class the report missed: the **TTS toggle at 88×26 on every route**, and **standalone nav/back links at 16–20px tall** across subject-picker, home, plan and badges. Fixed at the component level (`AppNavLink`, `StudentTtsToggle`) so it holds for future screens. The report's GMAT-bookmark claim was **false** — that button already uses `.touch-button` (`min-height: 52px`). | HIGH (a11y) | ✅ **Fixed (3.5.3)** |
...
| **D5** | **Duplicated components.** ✅ **Fixed in 3.5.4b:** the two "centered panel" shells (`LockedGradeScreen` → `CenteredPanel`), the four loading treatments (→ `LoadingPanel`), and the `!important` hero/chip resizes (→ `size` props). ⚠️ **The day-card merge: my 3.5.4c assessment was WRONG and is retracted.** I claimed it needed a product decision because "no per-day score exists in the subject model" — concluded from the *screen's markup*, not the storage contract. In fact `lib/english/storage.ts` and `lib/science/storage.ts` **persist `WorkbookProgressState`, the same days/sections shape as math**, so `percentDone` ([`lib/types/progress.ts:22`](../lib/types/progress.ts)) is already stored per day for every subject. The subject card simply never rendered it. **It is an ordinary refactor after all** — ✅ **done in 3.5.4d**: `DayCardShell` is now the single day-card design, with `DayCard` (math) and `SubjectHomeScreen` as thin wrappers. English/Science cards gained the progress bar, day number, medallion and state chip they always had the data for. The shell takes `rootTestId`/`ctaTestId` as props so each subject keeps its own testid namespace — **no spec changed**. **Lesson: read the data contract, not the JSX, before declaring something a product question.** | ~~MEDIUM~~ | ✅ **Fixed (3.5.4b + 3.5.4d)** |
| **D6** | **"Locked" is opacity on the whole card in 16 places**, at three different values (`opacity-60`, `opacity-50 grayscale`). **Measured 2026-07-18 (3.5.1):** muted hint text at `opacity-60` sits at **2.03:1** on white / **2.00:1** on cream — AA needs **4.5**. This is a hard failure, not a borderline one. Even `--muted` at *full* opacity on the locked surface only reaches **3.39:1**, so the fix needs its own darker token: `--locked-muted: #6f6880` measures **4.89:1** ✅. Tokens + `.is-locked` landed in 3.5.1; **adopted at all 9 genuine locked cards in 3.5.2a** (the other `opacity-60` sites were a disabled button and a deleted-user row, deliberately left alone). Measured on the rendered locked card: **2.03:1 → 4.89:1**. | ~~MEDIUM (a11y)~~ | ✅ **Fixed (3.5.1 + 3.5.2a)** |
| **D7** | **Admin area is off-palette** — `slate-*` text/borders/inputs make it look like a different product than the warm learner UI. **Fixed in 3.5.2b** (37 sites across AdminUsers + AdminProgress). | MEDIUM | ✅ **Fixed (3.5.2b)** |
| **D10** | **🔴 `--muted` itself failed WCAG AA — found while fixing D7.** At `#8a8298` it measured **3.67:1** on white / **3.54:1** on cream (AA needs 4.5), and `--muted-soft` at `#9a93a8` was worse at **2.95:1**. This affected *every* screen, not just admin. Discovered because the naive D7 fix — mapping admin's `text-slate-600` to `--muted`— would have **downgraded** admin from a passing **7.58:1** to a failing 3.67:1: a palette "cleanup" that made accessibility worse. **Fixed in 3.5.2b:** `--muted` → `#6f6880` (**5.29 / 5.12**), the *lightest* value on this hue that passes, so the visual change is the minimum necessary. `--muted-soft` deleted (one call site, worst ratio) and `--locked-muted` folded in — three greys became one. Verified on rendered elements against their real backdrops, not in the abstract. | HIGH (a11y) | ✅ **Fixed (3.5.2b)** |
...
| **D15** | ✅ **Fixed in 3.5.6.** All **132 `km.autogen.*` testids** across 10 components replaced with stable semantic ids in `lib/testIds.ts`. Repeated nodes are keyed by their domain id (`strandDayItem(strandId, n)`, `parentEntry(key)`, `rowUsername(userId)`) instead of a shared position index — previously every strand card shared `idx.22` and every user row shared `idx.18`. **The migration itself surfaced the exact bug the finding warns about:** collapsing `idx.5` onto `usernameInput()` put one id on both a `Field` wrapper and its `<input>`; caught by `getByTestId` throwing in the e2e that asserts on it, and fixed by giving the wrapper its own `usernameField()`. 16 test references updated to the new ids. | ~~MEDIUM (DX)~~ | ✅ **Fixed (3.5.6)** |
| **D16** | **The unit suite could report success without running every test.** Vitest prints "Test Files N passed (N)" from the count it *collected*; a file that fails to collect is simply absent from the total, so a run that executed 176 of 179 files still exits 0 and looks green. Observed during Phase 3.5: two consecutive runs of identical code reported 179/1472 and 176/1442, the 3 missing files unflagged. **Fixed in 3.5.6:** `scripts/check-test-collection.mjs` diffs the files Vitest reported (json reporter) against the `.test.tsx?` files on disk and fails on any shortfall; wired into `test:unit`. Proven to catch a simulated 176/179 gap and name the missing files. **The root *cause* of the intermittent under-collection is still unknown** — it did not reproduce in 5+ clean runs — but the guard converts the silent failure into a loud one regardless. | HIGH (test integrity) | ✅ **Guarded (3.5.6)** |
| **D14** | ✅ **Fixed in 3.5.4d.** `CenteredPanel` now renders its title as `<h1>` by default. Verified safe first: **every** call site is an early-return full-page state (404, locked, loading, PIN gate), so no page can produce a duplicate `<h1>`. Confirmed in-browser — `/this-page-does-not-exist` and `/admin` each report `h1Count: 1` where 404 previously had **zero**. `titleAs="div"` remains as an opt-down. | ~~MEDIUM (a11y)~~ | ✅ **Fixed (3.5.4d)** |
| ~~**D14**~~ | ~~**Most `CenteredPanel` screens render no heading at all.**~~ The shared panel renders its title as a `<div>`, so pages whose entire content *is* that panel (404, ComingSoon, several loading/locked branches) ship with **no `<h1>`** — a screen-reader user gets no page title. Surfaced in 3.5.4b while moving `LockedGradeScreen` onto the shell: that screen *did* have an `<h1>`, and consolidating naively would have silently downgraded it to a `<div>`. `CenteredPanel` gained an opt-in `titleAs="h1"` (default `div`, so no existing call site changed) and `LockedGradeScreen` uses it. **Remaining screens with `headings=0`: `not-found`, `ComingSoonScreen`, and the `DayOverviewScreen` / `SectionScreen` / `SubjectFinalExamScreen` / `SubjectSectionScreen` panel branches.** | MEDIUM (a11y) | ⬜ **Open — follow-up** |
| **D11** | **Fixing a token does not fix its hardcoded copies — 3.5.2b's AA claim was incomplete.** 3.5.2b corrected `--muted` and verified contrast on sampled `.muted` elements. But **35 sites hardcoded `#8a8298`** (the old failing value) and **43 hardcoded `#2c2348`**, bypassing the token entirely — including the shared components the QA report praised as the "good pole" (`Card`, `Tile`, `SectionHeader`, `CenteredPanel`). Those kept failing at 3.67:1. **Also found:** `--accent` (#8b75cc) fails as *text* at **3.81:1** (8 sites, incl. the TopBar login) — fixed by routing text usages to `--accent-strong` (**7.10:1**); as a background/border it is fine. **Fixed in 3.5.2c**, verified by a **full-page sweep of every leaf text node** (WCAG large-text rule applied), not a sample: **zero** solid-background failures remain. | HIGH (a11y) | ✅ **Fixed (3.5.2c)** |
| **D12** | **Primary CTA text fails AA on the accent gradient — needs a brand decision, not a sweep.** `.btn-accent` is `linear-gradient(135deg, #8b75cc, #a78bfa)` with white text: contrast runs **3.81:1 at the dark end to 2.72:1 at the light end** (AA needs 4.5 for 16px/600 text). This is the app's main call-to-action, on a children's product. Not auto-fixed in 3.5.2c because darkening the gradient changes the brand look — it is a product call, like the `--muted` darkening was. **Fixed in 3.5.4a (decision: darken both stops).** Gradient is now tokenized as `--cta-from: #6d4bc4` (**6.08:1**) → `--cta-to: #7c5fd0` (**4.77:1**), preserving the original dark→light direction. The *light* stop is the binding constraint. Two more white-on-`--accent` surfaces (Admin action button, Parent filter chip) moved to `--accent-strong` (**7.10:1**). **Follow-on:** `--muted` needed a second nudge `#6f6880` → `#6b647b` — the 3.5.2c value was measured only against white/cream and sat at **4.48:1** on the hero gradient, the darkest surface muted text lands on. | HIGH (a11y) | ✅ **Fixed (3.5.4a)** |
| **D13** | **Tailwind v4 compiled our documentation into the shipped stylesheet.** v4's automatic source detection runs *in addition to* the config's `content` array, so class names written as **prose** in `.md` (roadmap, `LEARNING_LOG`) became real CSS rules — including invalid `color: --token` from examples of the v3 syntax removed in 3.5.1b. Dead weight in production CSS, and it poisoned the "zero invalid declarations" build check used to verify D9. **Fixed in 3.5.2c** with `@source not "../**/*.md"`. | LOW | ✅ **Fixed (3.5.2c)** |
| **D8** | Smaller UX defects: next-badge banner `truncate`s its own description to nothing; badge tooltip clips off-screen on the first row; inline retry button wraps mid-line; DayCard shows its emoji twice; exam results nest a bordered card inside a bordered panel; PlanScreen uses brittle `km.autogen.*` testids. 🟡 **Mostly fixed:** next-badge `line-clamp-2` + duplicate DayCard emoji (3.5.4b); tooltip clipping, retry wrap, 11px tier label (3.5.4c); exam-results double border flattened (3.5.5b). ⬜ **NOT done — the `km.autogen.*` testids remain: 132 repo-wide** across 10 components (PlanScreen 46, AdminUsers 35, ProgressBar 15, …). These are auto-generated, position-derived ids that break whenever markup moves; replacing them with stable semantic ids is a **testability task in its own right**, not design QA, and is tracked as **D15**. | LOW–MEDIUM | 🟡 **Mostly fixed; testids → D15** |
...
- **Contract** — one state machine (`Companion`) exposing: `stage` (number 0…N, derived growth stage),
  `gradeTheme` (0=א׳/1=ב׳), `lookX`/`lookY` (−1…1, normalized cursor direction), `tap` (trigger),
  `celebrate` (trigger, fired once on stage-up), `idle` (boolean — reduced-motion / off-screen calm).
...
4. **Reduced-motion → static poster**; a **hide/off control** persists (WCAG 2.2.2).
5. Rive is **lazy-loaded** — no login-path / initial-bundle latency regression; CSP allows the wasm and is
   verified against the Report-Only report.
6. New key `kids_math.companion.v1` only; **no `lib/*/storage.ts` change**; `multi-user-isolation` +
   `auth-backward-compat` anchors green.
7. Ships **flag-gated** with a placeholder `.riv` behind the documented input contract; real art integrates
...
| 3 | **Account lifecycle, data export & governance** | MAX | ✅ go/no-go given 2026-07-18 | ✅ **Done** ([#104](https://github.com/RoieArgaman/kids_math/pull/104)) — soft delete (not erasure), admin export, retention selector, `firestore.rules`. Closed **🟡 partial** as planned: C2a only; **C2b → Phase 4** |
