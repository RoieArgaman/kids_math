...
| **D15** | **132 auto-generated `km.autogen.*` testids** across 10 components (PlanScreen 46, AdminUsers 35, ProgressBar 15, LoginModal/DayHeader 8 each, …). They encode a node's *position* (`…node.idx.15`), so any markup change silently repoints them at a different element — a test can keep passing while asserting on the wrong node. Split out of D8 in 3.5.5b: this is a **testability/DX task**, not design QA, and wants its own pass with matching spec updates. | MEDIUM (DX) | ⬜ **Open — own task** |
