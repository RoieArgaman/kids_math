...
| **C9** | **Cross-region + far-from-users topology:** app ran in `us-east4` (Virginia), Firestore in `europe-west1` (Belgium), users in Israel — every DB call crossed the Atlantic (~4/login). Login p95 ≈ 16s under burst. **Fixed 2026-07-18:** relocated the app to **`europe-west4`** (Netherlands — nearest App Hosting region to Firestore's europe-west1; europe-west1 itself isn't an App Hosting region) via a new backend `kids-math-eu`, cutting over to `kids-math-eu--…europe-west4.hosted.app`. With `minInstances:3`, load test showed **login median ~676ms / p95 ~3.73s** (from ~16s) and progress-push p95 ~887ms; `TRUSTED_PROXY_HOPS=2` + enforcement re-verified on the new backend. See [`C9_RELOCATION_RUNBOOK.md`](../.claude/docs/C9_RELOCATION_RUNBOOK.md). **Remaining after soak: decommission old `us-east4` backend; enable the weekly load-test `schedule` + tighten the k6 login threshold.** | HIGH | Cutover shipped; soak + decommission pending |
