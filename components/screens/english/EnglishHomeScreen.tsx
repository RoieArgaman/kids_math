...
export function EnglishHomeScreen() {
  ...
  useEffect(() => {
    ...
    setCompleteMap(map);
    setIsHydrated(true);
    // days is module-stable; safe to run once.
  ...
  return (
    <main data-testid={root} className="pb-10">
      ...
      <HeroHeader
        ...
        title="אַנְגְּלִית 🔤"
        subtitle="מַקְשִׁיבִים, בּוֹחֲרִים, וְלוֹמְדִים מִילִּים חֲדָשׁוֹת."
        decorations={[
          ...
          <div data-testid={childTid(testIds.screen.english.home.examCard(), "row")} className="flex items-center justify-between gap-3">
            ...
            {examUnlocked ? (
              <Link
                ...
                href={routes.englishExam({ previewAll })}
                className="touch-button btn-accent rounded-2xl px-5 py-3 text-sm font-semibold shadow-sm"
              >
                לַמִּבְחָן
              </Link>
