...
import { scienceLevelLabel, scienceLevelSubtitle, isScienceLevelUnlocked } from "@/lib/science/levels";
...
export function ScienceHomeScreen({ level }: { level: ScienceLevel }) {
  ...
  useEffect(() => {
    ...
    if (!isScienceLevelUnlocked(level, { previewAll: preview })) {
      router.replace(routes.scienceLevelPicker({ previewAll: preview }));
      return;
    }
  ...
  const root = testIds.screen.science.home.root();

  return (
    <main data-testid={root} className="pb-10">
      <div data-testid={childTid(root, "nav")} className="mb-4">
        <AppNavLink href={routes.scienceLevelPicker({ previewAll })}>חֲזָרָה לִבְחִירַת שָׁלָב</AppNavLink>
      </div>

      <HeroHeader
        data-testid={testIds.screen.science.home.hero()}
        title={`מַדָּעִים · ${scienceLevelLabel(level)}`}
        subtitle={scienceLevelSubtitle(level)}
        decorations={[
      ...
      <section data-testid={childTid(root, "grid")} className="flex flex-col gap-4">
        {days.map((day, idx) => {
          ...
          const isLocked =
            isHydrated && idx > 0
              ? !canUnlockNextDay(previousDay!, loadScienceProgressState().days[previousDay!.id])
              : !previousComplete && idx > 0;
          const cardId = testIds.screen.science.home.dayCard(day.id);
            ...
            >
              <div data-testid={childTid(cardId, "row")} className="flex items-center justify-between gap-3">
                ...
                {isLocked ? (
                  ...
                  <Link
                    data-testid={testIds.screen.science.home.dayCardCta(day.id)}
                    href={routes.scienceDay(level, day.id)}
        ...
        <div
          data-testid={testIds.screen.science.home.examCard()}
          className={`rounded-3xl border-2 p-5 shadow-sm transition-all ${
            examUnlocked ? "border-[#e7defb] bg-[#ede9fe]" : "border-[#eceaf1] bg-[#faf9fc] opacity-60"
          }`}
        >
          <div data-testid={childTid(testIds.screen.science.home.examCard(), "row")} className="flex items-center justify-between gap-3">
            <div data-testid={childTid(testIds.screen.science.home.examCard(), "info")} className="min-w-0 flex-1">
              <p data-testid={childTid(testIds.screen.science.home.examCard(), "title")} className="text-base font-bold leading-tight">
                📝 מִבְחָן מְסַכֵּם · {scienceLevelLabel(level)}
              </p>
              <p data-testid={childTid(testIds.screen.science.home.examCard(), "objective")} className="mt-0.5 text-xs text-[#8a8298]">
                נִפְתָּח אַחֲרֵי שֶׁמַּשְׁלִימִים אֶת כָּל הַשִּׁעוּרִים בַּשָּׁלָב.
              </p>
            </div>
            {examUnlocked ? (
              <Link
                data-testid={testIds.screen.science.home.examCardCta()}
                href={routes.scienceExam(level, { previewAll })}
                className="touch-button btn-accent rounded-2xl px-5 py-3 text-sm font-semibold shadow-sm"
              >
                לַמִּבְחָן
              </Link>
            ) : (
              <span data-testid={childTid(testIds.screen.science.home.examCard(), "lockedHint")} className="text-sm font-semibold text-[#8a8298]">
                🔒 נָעוּל
              </span>
            )}
          </div>
