...
export function ScienceDayScreen({ level, dayId }: { level: ScienceLevel; dayId: DayId }) {
  ...
  const { markComplete, percentDone, correctAnswers } = useProgress(dayId, { subject: "science" });
  ...
  if (!day) {
    return (
      <main data-testid={testIds.screen.science.day.root(`${dayId}.not-found`)}>
        <CenteredPanel
          emoji="🔍"
          title="הַשִּׁעוּר לֹא נִמְצָא."
          actions={
            <ButtonLink href={routes.scienceHome(level)} className="w-full text-center">
              חֲזָרָה לְמַדָּעִים
            </ButtonLink>
          }
        />
  ...
  const root = testIds.screen.science.day.root(dayId);
  ...
  return (
    <main data-testid={root}>
      <div
        data-testid={testIds.screen.science.day.nav(dayId)}
        className="mb-3 flex flex-wrap items-center justify-between gap-3"
      >
        <AppNavLink href={routes.scienceHome(level)}>חֲזָרָה לְמַדָּעִים</AppNavLink>
        <AppNavLink href={routes.scienceLevelPicker()}>חֲזָרָה לִבְחִירַת שָׁלָב</AppNavLink>
      </div>
      ...
      <div data-testid={childTid(root, "sections")} className="mb-6 flex flex-col gap-4">
        {day.sections.map((section, idx) => {
          ...
          const cardRoot = testIds.screen.science.day.sectionCard(dayId, section.id);
            ...
            >
              ...
              <div data-testid={childTid(cardRoot, "bottomRow")} className="flex items-center justify-between gap-3">
                ...
                {isCardLocked ? (
                  ...
                  <Link
                    data-testid={testIds.screen.science.day.sectionCardCta(dayId, section.id)}
                    href={routes.scienceSection(level, dayId, section.id)}
      ...
      {allSectionsComplete && (
        <div
          data-testid={testIds.screen.science.day.completionPanel(dayId)}
          className="mb-6 rounded-3xl border border-[#bbf7d0] bg-gradient-to-br from-[#f0fdf4] to-[#d1fae5] p-6 text-center shadow-md"
        >
          <p data-testid={childTid(testIds.screen.science.day.completionPanel(dayId), "icon")} className="mb-1 text-5xl">
            🎉
          </p>
          <p
            data-testid={childTid(testIds.screen.science.day.completionPanel(dayId), "title")}
            className="mb-4 text-2xl font-semibold text-[#047857]"
          >
            כָּל הַכָּבוֹד!
          </p>
          <button
            data-testid={testIds.screen.science.day.completeCta(dayId)}
            type="button"
      ...
      <StarReward
        ...
        onConfirm={() => {
          setShowReward(false);
          router.push(routes.scienceHome(level));
        }}
      />
    </main>
  );
