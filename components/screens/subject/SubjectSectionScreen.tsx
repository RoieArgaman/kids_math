...
/**
 * Shared subject Section screen (English + Science). All subject-specific data —
 * content loader, progress subject, routes, labels, testids — comes from
 * `config`; the markup here is identical to what each subject used to render.
 */
export function SubjectSectionScreen({
  config,
  level,
  dayId,
  sectionId,
  ...
  const { setAnswer, percentDone, sectionWrongCount, correctAnswers, resetSection } = useProgress(
    dayId,
    { subject: config.progressSubject, sectionId },
  );
  ...
  const ids = config.section.testIds;

  if (!day || !section) {
    return (
      <main data-testid={ids.root(dayId, `${sectionId}.not-found`)}>
        <CenteredPanel
          emoji="🔍"
          title="הַחֵלֶק לֹא נִמְצָא."
          actions={
            <ButtonLink href={config.dayRoute(level, dayId)} className="w-full text-center">
              חֲזָרָה לַשִּׁעוּר
            </ButtonLink>
          }
        />
  ...
  if (sectionIdx > 0) {
    ...
    if (blocked) {
      return (
        <main data-testid={ids.root(dayId, `${sectionId}.locked`)}>
          <CenteredPanel
            emoji="🔒"
  ...
  const stickyHeaderId = ids.stickyHeader(dayId, sectionId);
  const completionPanelId = ids.completionPanel(dayId, sectionId);
  const sectionRootId = ids.root(dayId, sectionId);

  return (
    <main data-testid={sectionRootId}>
      <div
        data-testid={ids.nav(dayId, sectionId)}
        className="mb-3 flex flex-wrap items-center justify-between gap-3"
      >
        <AppNavLink href={config.dayRoute(level, dayId)}>חֲזָרָה לַשִּׁעוּר</AppNavLink>
      ...
      {resetNotice ? (
        <div
          ...
          role="alert"
          className="mb-5 mt-3 rounded-2xl border border-[#fecdd3] bg-[#fff5f6] p-4 text-sm font-semibold text-[#b91c1c] shadow-xs"
        >
          ⚠️ {resetNotice}
        </div>
      ) : null}
