...
export function NumberLine({ start, end }: NumberLineProps) {
  ...
  return (
    <div data-testid="km.autogen.numberline.node.idx.0" className="rounded-2xl border border-[#cde4f3] bg-[#f5fbff] p-3" dir="ltr">
      <div data-testid="km.autogen.numberline.node.idx.1" className="relative flex items-end justify-between gap-2">
        ...
        {points.map((point) => (
          <div data-testid="km.autogen.numberline.node.idx.3" key={point} className="relative flex flex-1 flex-col items-center">
            <div data-testid="km.autogen.numberline.node.idx.4" className="h-4 w-0.5 bg-[#5b9fcc]" />
            <span data-testid="km.autogen.numberline.node.idx.5" className="mt-1 text-sm font-bold text-[#0c4a6e]">{point}</span>
          </div>
        ))}
      </div>
    </div>
